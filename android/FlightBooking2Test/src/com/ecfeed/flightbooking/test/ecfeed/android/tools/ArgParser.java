...
package com.ecfeed.flightbooking.test.ecfeed.android.tools;
...
public class ArgParser {
	...
	public Invocable createMethodToInvoke(Object object, String testArguments) {
		return createMethodToInvoke(object, decodeArguments(testArguments), false);
	}
	
	public Invocable createMethodToInvokeWithErrorCodes(Object object, String testArguments) {
		return createMethodToInvoke(object, decodeArguments(testArguments), true);
	}	
	
	private String decodeArguments(final String testArguments) {
		StringBuilder builder = new StringBuilder();
		int argsLength = testArguments.length();
		
		for (int index = 0; index < argsLength; index++) {
			if (isEscapeWithUnderlineAt(index, testArguments)) {
				builder.append(UNDERLINE);
				index++;
				continue;
			}
			if (isUnderlineAt(index, testArguments)) {
				builder.append(SPACE);
				continue;
			}
			builder.append(testArguments.charAt(index));
		}
		
		return builder.toString();
	}
	
	private boolean isEscapeWithUnderlineAt(int index, String strg) {
		if (index >= strg.length()) {
			return false;
		}
		if (strg.charAt(index) != "\\".charAt(0)) {
			return false;
		}
		return isUnderlineAt(index+1, strg);
	}
	
	private boolean isUnderlineAt(int index, String strg) {
		if (strg.charAt(index) != "_".charAt(0)) {
			return false;
		}
		return true;
	}
	...
	private class ParamTypeWithArg {
		...
		private Object convertEnum() throws RuntimeException {
			...
			Class<?> theClass = null;
			try {
				theClass = Class.forName(fParameterType);
			} catch (ClassNotFoundException e) {
